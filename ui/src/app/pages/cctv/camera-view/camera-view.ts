...
export class CameraViewComponent implements OnInit, OnDestroy {
    ...
    @ViewChild(NzImageDirective)
    streamImage: NzImageDirective;
    ...
    ngOnInit() {
        this.route.paramMap
            .pipe(
                ...
                map(params => params.get('camera')),
                mergeMap(camID => {
                    if (!!camID) {
                        return this.cctvapi.listCameras()
                            .pipe(map(cams => cams.find(cam => cam.id === camID)));
                    }
                    return of(null as LocalCamera);
                })
            )
            .subscribe(cam => {
                this.cam = cam || null;
                this.headerTitle.set(cam?.name || '');
            });
