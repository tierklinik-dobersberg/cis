...
export class QuickTimeSelectorComponent implements OnInit, OnDestroy {
  ...
  get resourcesRequired() {
    return Array.from(this._resourcesRequired);
  }
  ...
  selectRangeStart(slot: TimeSlot, user: string, downEvent: MouseEvent) {
    ...
    const start = (downEvent.target as HTMLSpanElement).getBoundingClientRect()
      .x;
    ...
    const stopMouseMove = this.renderer.listen(
      document,
      'mousemove',
      (event) => {
    ...
    updateSlots$.pipe(takeUntil(this.destroy$)).subscribe(
      (event) => {
    ...
    combineLatest([
      this.onDate$.pipe(
        mergeMap((d) => {
          if (
            !!this.selectedStart &&
            this.selectedStart.toDateString() !== d.toDateString()
          ) {
            this.selectedStart = null;
            this.selectedEnd = null;
          }
          ...
          return forkJoin({
            date: of(d),
            roster: this.loadOrNull(this.rosterapi.forMonth(d)),
            openinghours: this.loadOrNull(
              this.openinghoursapi.getOpeningHours(d)
            ),
            events: this.loadOrNull(this.calendarapi.listEvents(d)),
          });
        })
      ),
  ...
  private updateTimeSlots(
    ...
    let [allUsers, unassignedCalendars, hasRoster] = this.getAllAvailableUsers(
      day,
      config
    );
    ...
    let [earliestDate, latestDate] = this.getEarliestAndLatestDate(
      openinghours,
      events
    );
    ...
    const isInFrame = (start: Date, end: Date) => {
      ...
      return openinghours.some(
        (frame) =>
          frame.from.getTime() <= tsStart && frame.to.getTime() >= tsEnd
      );
    };
    ...
    this.timeSlots = [];
    for (
      let iter = earliestDate;
      iter.getTime() < latestDate.getTime();
      iter = this.slotSize.addTo(iter)
    ) {
      ...
      if (hasRoster) {
        ...
        usersSlot = [...usersSlot, ...unassignedCalendars];
      } else {
      ...
      const allResourcesFree = !Object.keys(users).some((key) => {
        return users[key].some(
          (event) =>
            !!event.data?.requiredResources?.some((res) =>
              this._resourcesRequired.has(res)
            )
        );
      });
  ...
  private getEarliestAndLatestDate(
    hours: OpeningHour[],
    events?: LocalEvent[]
  ): [Date, Date] {
    ...
    events?.forEach((event) => {
      if (
        earliestDate === null ||
        event.startTime.getTime() < earliestDate.getTime()
      ) {
      ...
      if (
        latestDate === null ||
        event.endTime?.getTime() > latestDate.getTime()
      ) {
    ...
    hours.forEach((frame) => {
      if (
        earliestDate === null ||
        frame.from.getTime() < earliestDate.getTime()
      ) {
  ...
  private getAllAvailableUsers(
    day?: Day,
    config?: UIConfig
  ): [SlotUser[], string[], boolean] {
      ...
      (config.UI?.CreateEventAlwaysAllowCalendar || []).forEach((calID) => {
        ...
        if (!cal) {
          this.nzMessageService.error(
            'Kalender mit ID ' + calID + ' nicht gefunden'
          );
          return;
        }
