  ...
  template: `
    ...
    <div class="flex flex-col">
      @if (!!header.breadcrumps) {
        <div class="text-color-primary-dark text-opacity-75 flex flex-row items-center leading-3 text-xs font-lato">
      ...
      @if (header.description) {
        <span class="hidden text-sm break-normal text-secondary font-iter sm:block">{{ header.description }}</span>
      }
    </div>
    `,
  styles: [
