...
describe('Comment API', () => {
    it('should return comments by key', async () => {
        const response = await Alice.get('/api/comments/v1/roster:2021-5');
        expect(response.status).toBe(200);
        expect(response.data).toHaveSize(2);
    })

    it('should return an empty array instead of 404', async () => {
        const response = await Alice.get('/api/comments/v1/roster:2021-3');
        expect(response.status).toBe(200);
        expect(response.data).toHaveSize(0);
    })

    it('should be able to search comments by prefix', async () => {
        const response = await Alice.get('/api/comments/v1/roster:2021', {
            params: {
                prefix: true,
            }
        });
    ...
    it('should be possible to create new comments', async () => {
        ...
        const response = await Alice.post('/api/comments/v1/' + key, JSON.stringify("Test comment message"), {
            headers: {
        ...
        const list = await Alice.get('/api/comments/v1/' + key)
    ...
    it('should be possible to reply to comments', async () => {
        const response = await Alice.put('/api/comments/v1/comment/6092e75fb832d4592661eed8/replies', "reply message", {
            headers: {
        ...
        const list = await Alice.get('/api/comments/v1/roster:2021-5');
        expect(list.status).toBe(200);
