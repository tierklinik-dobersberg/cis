...
describe("Comment API", () => {
  ...
  it("should be possible to create new comments", async () => {
    ...
    const response = await Alice.post(
      "/api/comments/v1/" + key,
      JSON.stringify("Test comment message"),
      {
        headers: {
    ...
    expect(new Date(list.data[0].createdAt).getTime() / 1000).toBeCloseTo(
      now.getTime() / 1000,
      0
    );
  });
  ...
  it("should be possible to reply to comments", async () => {
    const response = await Alice.put(
      "/api/comments/v1/comment/6092e75fb832d4592661eed8/replies",
      "reply message",
      {
        headers: {
