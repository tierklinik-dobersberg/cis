...
import { Alice, expectResponse, Unauth } from '../utils';
...
describe("The Identity API:", () => {
    ...
    describe("user, role and permissions management", () => {
        ...
        it("should be possible to add a permission to the user", async () => {
            ...
            const response = await Alice.post("/api/identity/v1/permissions/users/lucy", perm)
            expect(response.status).toBe(200, response.data)
        ...
        it("should be possible to remove a permission from a user", async () => {
            const response = await Alice.delete(`/api/identity/v1/permissions/users/lucy/${userPermId}`)
        ...
        it("should be possible to create a role", async () => {
            ...
            const response = await Alice.post("/api/identity/v1/roles/write-access", role)
            expect(response.status).toBe(204, response.data)

            const roleResponse = await Alice.get("/api/identity/v1/roles/write-access")
        ...
        it("should be possible to add a permission to a role", async () => {
            ...
            const response = await Alice.post("/api/identity/v1/permissions/roles/write-access", perm)
            expect(response.status).toBe(200, response.data)
            expect(response.data.id).toBeTruthy()
        ...
        it("should be possible to remove a permission from a role", async () => {
            const response = await Alice.delete(`/api/identity/v1/permissions/roles/write-access/${rolePermId}`)
            expect(response.status).toBe(204, response.data)
        ...
        it("should be possible to assign a role to a user", async () => {
            const response = await Alice.put("/api/identity/v1/users/lucy/roles/write-access")
            expect(response.status).toBe(204, response.data)
        ...
        it("should be possible to unassign a role from a user", async () => {
            const response = await Alice.delete("/api/identity/v1/users/lucy/roles/write-access")
            expect(response.status).toBe(204, response.data)
        ...
        it("should be possible to remove a role", async () => {
            const response = await Alice.delete("/api/identity/v1/roles/write-access")
            expect(response.status).toBe(204)
