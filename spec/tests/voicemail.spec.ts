...
describe('VoiceMail API', () => {
    ...
    it("should support listing voice-mails", async () => {
        const response = await Alice.get('http://localhost:3000/api/voicemail/v1/list')
    ...
    it("should support marking voice-mails as seen", async () => {
        ...
        const response = await Alice.put('http://localhost:3000/api/voicemail/v1/recording/6020219cfbdac8b50bb5d226/seen')
    ...
    it("should support marking voice-mails as unseen", async () => {
        expect((await getVoicemail("6020219cfbdac8b50bb5d226"))?.read).toBeTrue()
        const response = await Alice.delete('http://localhost:3000/api/voicemail/v1/recording/6020219cfbdac8b50bb5d226/seen')
        expect(response.status).toBe(204)
        expect((await getVoicemail("6020219cfbdac8b50bb5d226"))?.read).toBeUndefined()
    })

    it("should support retrieving voicemails by ID", async () => {
        const response = await Alice.get("http://localhost:3000/api/voicemail/v1/recording/60201fd71019fe0e812caa87/")
        expect(response.status).toBe(200);
