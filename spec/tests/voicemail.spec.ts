...
describe("VoiceMail API", () => {
  ...
  it("should support marking voice-mails as seen", async () => {
    expect(
      (await getVoicemail("6020219cfbdac8b50bb5d226"))?.read
    ).toBeUndefined();
    const response = await Alice.put(
      "/api/voicemail/v1/recording/6020219cfbdac8b50bb5d226/seen"
    );
  ...
  it("should support retrieving voicemails by ID", async () => {
    const response = await Alice.get(
      "/api/voicemail/v1/recording/60201fd71019fe0e812caa87"
    );
    expect(response.status).toBe(200, response.data);
