...
describe('APIs for 3CX', () => {
    describe("CRM integration", () => {
        it('retrieving contact using +43', async () => {
            const response = await Alice.get(`/api/external/v1/contact?phone=%2B4333333333`)
        ...
        it('retrieving contact using 0043', async () => {
            const response = await Alice.get(`/api/external/v1/contact?phone=004333333333`)
            expect(response.status).toBe(200)
            expect(response.data.contact.phone1).toBe('+4333333333')
            expect(response.data.contact.phone2).toBe('33333333')
        })
        it('retrieving contact using national number', async () => {
            const response = await Alice.get(`/api/external/v1/contact?phone=33333333`)
            expect(response.status).toBe(200)
            expect(response.data.contact.phone1).toBe('+4333333333')
            expect(response.data.contact.phone2).toBe('33333333')
        })
        it('should return with 404 for unknown numbers', async () => {
            ...
            const response = await Alice.get(`/api/external/v1/contact?phone=99999999`).catch(err => err.response as AxiosResponse)
    ...
    describe("call journaling", () => {
        it("should be possible to record an unidentified caller", async () => {
            const response = await Alice.post(`/api/external/v1/calllog?ani=33333333&did=99`)
            expect(response.status).toBe(204);
        })
        it("should be possible to create an unidentified anonymous caller", async () => {
            const response = await Alice.post(`/api/external/v1/calllog?ani=Anonymous&did=99`)
            expect(response.status).toBe(204);
        })

        it("should be possible to create an identified call record", async () => {
            const response = await Alice.post(`/api/external/v1/calllog`, {
                duration: "10",
                number: "12341234",
                agent: "10",
                callType: "Inbound",
            ...
            const loadResponse = await Alice.get(`/api/calllogs/v1/customer/vetinf/3`)
        ...
        it("should replace an unidentified record with an identified one", async () => {
            const now = new Date();
            const response = await Alice.post(`/api/external/v1/calllog?ani=88888888&did=99`)
            expect(response.status).toBe(204);

            const [year, month, day] = [now.getFullYear(), now.getMonth() + 1, now.getDate()]
            let callLogsFromToday = await Alice.get(`/api/calllogs/v1/date/${year}/${month}/${day}`)
            ...
            const response2 = await Alice.post(`/api/external/v1/calllog`, {
                duration: "10",
                number: "88888888",
                agent: "10",
                callType: "Inbound",
            ...
            callLogsFromToday = await Alice.get(`/api/calllogs/v1/date/${year}/${month}/${day}`)
            expect(callLogsFromToday.status).toBe(200)
